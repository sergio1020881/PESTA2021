...
\block{Informação e Controlo}{
	Tratamento dos valores medidos, conversão de sinal medido pelo sensor para analógico e depois electrico, conversão de analógico para digital usando medidas discretas, tirando varias amostras para obter uma média depois dedução de sua massa.\\
	Um \textit{Frontend} de controlo intuitivo, com respectivo offset e meio de calibração para a celula que estiver a ser usada.\\
	Uma balança electrónica simples.
	\vspace{.1cm}
}
\end{columns}
\begin{columns}
...
\block{Conclusões}{
	...
	\coloredbox{
		...
		\begin{minipage}[b!]{.8\linewidth}
			explain eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
			\vfill
		\end{minipage}	
	}
	\bigskip
...
\note[targetoffsetx=-34cm, targetoffsety=-8cm, width=.3\linewidth]{
