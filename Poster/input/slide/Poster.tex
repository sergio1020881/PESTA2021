...
\block{Introdução}{
	...
	O que vai ser utilizado no projeto vai ser um \textbf{célula de carga}, estas células tem quatro sensores \textit{strain gauges} ligadas em ponte wheatstone que vão detetar a distorção (pressão) do material, ou seja, a célula de peso e gerar um sinal em tensão proporcional a força exercida quando alimentado. Seque o mesmo principio de uma mola [ $K= \frac{\Delta l}{F}$ ].
}
\end{columns}
\begin{columns}
\column{0.1}
...
\block{Kit Desenvolvimento}{
	\begin{tikzfigure}
		\includegraphics[width=0.65\textwidth]{./image/PESTA/kit/Kit_Desenvolvimento_2.jpg}
	\end{tikzfigure}
}
\end{columns}
\begin{columns}
...
\block{Sensor}{
	...
	Neste projecto é o caso de utilizar uma celula de carga, ou seja, sensores piezoresistivos para determinar a massa dos objectos.
