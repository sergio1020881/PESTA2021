...
Pretende-se neste projeto então recorrendo aos componentes disponíveis no mercado elaborar uma balança digital, os componentes necessários vai ser um micro controlador como cérebro do sistema, ter meios de interface humana especificamente botões, \textit{leds} indicadores e um \textit{display} para visualização das leituras e valore de calibração, também uma fonte de alimentação para suportar as cargas.
Para a medição da grandeza em estudo vai ter que se usar um sensor que mede o peso dos objetos e um amplificador de sinal com resolução suficiente para adquirir umas leituras com maior precisão.
\\
\\
...
Para medir a massa, recorreu-se a uma \textbf{célula de carga} de 50 Kg, apresentada na \textit{figura} \ref{Diagrama-bloco-1}, que determina a pressão exercida por um dado objeto. Os sensores (\textit{strain gauges}) são instalados sobre um bloco de alumínio como indicado na \textit{figura} \ref{Load_Cell_1}. A célula de carga utiliza sensores piezoresistivos numa montagem em ponte \textit{Wheatstone}, sobre essa superfície em locais específicos. %explicar melhor esta parte da montagem
\\
\begin{figure}[H]
...
\end{figure}
\figurespace{.5}
A placa \textit{Load Cell Amplifier } pode ser fisicamente configurada para determinar o número de amostras por segundo a ser transmitido. Este circuito tem duas opções: a opção de \textcolor{blue}{10} amostras por segundo, e de \textcolor{blue}{80} amostras por segundo. Neste projeto foi utilizada a segunda opção, que necessita de uma pequena configuração na placa de circuito de impresso, isto é, é necessário abrir o respetivo \textit{jumper} de configuração.
\\
\\
...
\newpage
Abaixo a \textit{figura} \ref{SPS_64} que foi adquirida do trabalho realizado recorrendo ao osciloscópio da RIGOL DS1102E e seu software \textit{Rigol Waveform Viewer}, demonstrando a comunicação entre o amplificador de sinal e o micro controlador do valor \acs{adc}, a linha amarela é a informação (\acs{adc}) e a azul os impulsos do \textit{clock} PD\_SCK. Neste exemplo é visível cinco conversões executadas.
\begin{figure}[H]
	\centering
...
O \ac{lcd} utilizado é de 4x20, isto é, quatro linhas de vinte caracteres cada, tornando-se no interface humano principal, e também durante o este projeto numa ferramenta extremamente útil para fazer o \textit{debug} e executar testes ao código.\\
Este \acs{lcd} é usado para visualizar as leituras e o valor de calibração da célula de carga, a escolha é unicamente por ter este componente no meu stock de material com uma biblioteca que o suporta, uso de quatro linhas apenas para facilitar na deteção de anomalias e testar o código, podendo visualizar vários parâmetros ao mesmo tempo.
\\
\\
%que projectos ?
%mencionar relatórios e mencionar relatorio LABSIS.
Uma biblioteca já feita para outros projetos pessoais que serviu para aplicar na disciplina de laboratório de sistemas e neste, poupando bastante tempo, revelando a importância de documentar os conhecimentos adquiridos. A biblioteca está presente no \textit{anexado} \ref{lcd-h} e \ref{lcd-c}.
%[\ref{codigo}]
\\
\\
Abaixo está a tabela \ref{LCD_connections}, disponível com as respetivas ligações, e uma imagem de um \acs{lcd} 4x20 azul na \textit{figura} \ref{4x20_LCD}.
\tablespace{.2}
\begin{table}[H]
	\centering
...
\\
\\
Os micro-controladores da AVR utilizam uma arquitetura de instruções \ac{risc} que reduz a complexidade dos circuitos na codificação de cada instrução. Daí que os micro-controladores que se baseiam nestes tipos de arquitetura são sinónimo de código reduzido, alta performance e baixo consumo energético.
\\
\\
\begin{figure}[H]
	\centering
