...
Para a medição da grandeza em estudo vai ter que se usar um sensor na qual mede o peso dos objetos e um amplificador de sinal com resolução suficiente para adquirir umas leituras com maior precisão.
\\
\\
O kit de desenvolvimento da \textit{figura} \ref{Kit_Desenvolvimento_2} permitiu a realização de testes, com vista à validação do projeto, bem como à execução de alterações e melhoramentos.
\\
\\
...
\end{minipage}
\minipagespace{.5}
Normalmente nesta configuração só são usados um ou dois sensores, e estes podem estar ligados nos extremos opostos, ou ligados ao mesmo ponto da alimentação, só em casos muito raros são utilizados quatro sensores na qual sua sensibilidade é máxima. Se os valores das quatro resistências são iguais ou nos casos em que $R_2 R_3 = R_4 R_1$, então a tensão $V_{AB}$ na saída é nula.
\\
\\
...
\\
\\
Abaixo a \textit{figura} \ref{SPS_64} que foi adquirida do trabalho realizado, uma demonstração da comunicação entre o amplificador de sinal e o Micro controlador, onde a linha amarela é a informação e a azul os impulsos do relógio.
\begin{figure}[H]
	\centering
...
Uma biblioteca já feita para outros projetos pessoais serviu para aplicar na disciplina de laboratório de sistemas e neste, poupando bastante tempo, revelando a importância de documentar os conhecimentos adquiridos. A biblioteca está presente no \textit{anexado} \ref{lcd-h} e \ref{lcd-c}.
%[\ref{codigo}]
\\
\\
...
Os micro-controladores da Atmel de 8 a 32 \textit{bits} são baseados na arquitetura avançada de Harvard, na qual está concebido para baixos consumos e alta performance.
\\
\\
...
\newpage
As características do micro controlador Atmega 128 estão abaixo indicadas na lista. Dos componentes que constituem o Atmega 128, os temporizadores são os que considero os mais importantes
%porque
...
Características DC/DC:
	...
	\end{itemize}
\end{minipage}
Como a \textit{mainboard} tem um regulador de tensão de 5 Volt, para aumentar a eficiência  e ter uma alimentação variável de entrada foi utilizado o \textit{buck converter} da \textit{figura} \ref{buck-converter}. Como podemos ver nas características.
%explicar melhor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Software}
\section{Software}
