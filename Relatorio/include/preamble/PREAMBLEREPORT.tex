\documentclass[titlepage, a4paper, 12pt, reqno, openany]{report}
%\documentclass[LEEC, portuguese,]{DEEclass}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{titlepic}
\usepackage[title,titletoc]{appendix}
%\usepackage[title,toc,titletoc,page]{appendix}
%%%%%%encoding%%%%%%
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
%\usepackage[acronym,toc,shortcuts,nopostdot,nogroupskip,automake,nonumberlist]{glossaries}
\usepackage[shortcuts, acronym, toc, nogroupskip, nonumberlist]{glossaries}
\usepackage[portuguese]{babel}
\usepackage{babelbib}
\usepackage{hyphenat}
%%%%%%%%%%%%%%%%%%%%%%Hyphenation rules%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage[top=2.5cm,left=3cm,right=2.5cm,bottom=2.5cm]{geometry}
%\usepackage[left=2cm,top=1cm,bottom=2cm,right=3cm,nohead,nofoot]{geometry}
\usepackage{graphicx} %permite inserir figuras
\usepackage{subcaption}
\usepackage[font=small,labelfont=bf]{caption} %reference figures table
\usepackage{color,colortbl}
\usepackage[usenames,dvipsnames,svgnames,table]{xcolor} %permite letras coloridas
\usepackage{paralist}
\usepackage{float}
%\usepackage{fancyvrb}
\usepackage{verbatim}
\usepackage{moreverb}
\usepackage{lipsum}
\usepackage{lscape}
\usepackage{multicol}
\usepackage{multirow}
\usepackage{makecell}
\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{mathrsfs}
\usepackage{float}
\usepackage{pgfgantt}
\usepackage{rotating}
\usepackage[graphicx]{realboxes}
\usepackage{tabularx}
\usepackage{listings}
\usepackage[nottoc]{tocbibind} %% addiciona lista figuras e tabelas a toc
%%%ISEP%%%
\usepackage{lmodern}
\usepackage{calc}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage{array}
\usepackage[export]{adjustbox}
\usepackage{url}
\usepackage{setspace} %distancia entree linhas
\usepackage{eurosym}
\usepackage{romannum}
\usepackage[en-US]{datetime2}
\usepackage{enumitem}
\usepackage{enumerate}
\usepackage{tikz}
\usepackage{circuitikz}
\usetikzlibrary{matrix,shapes.geometric,arrows,trees,positioning,calc}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\makeindex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibliographystyle{plain}
\selectbiblanguage{portuguese}
\setbtxfallbacklanguage{english}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
