...
\begin{minipage}[!b]{.45\linewidth}
	Ao lado a \autoref{Microchip-Studio} nos mostra o logo da empresa, e abaixo a \autoref{Work-Space} demonstra o ambiente de trabalho onde se executa todo o código que vai correr no microcontrolador.
	\\
	\\
	\\
	\\
...
\begin{minipage}{.6\linewidth}
	Ao lado (\autoref{Bibliotecas}) estão as bibliotecas usadas neste projeto, as que estão dentro da caixa amarela são as que foram criadas pessoalmente.
...
\\
\\
O código que executa a rotina de leitura \acs{adc} é demonstrado na \autoref{HX711-read-raw}, sendo que esta é chamada pela interrupção periódica da \autoref{Interrupt_1}, em que só é ativa quando o resultado da função na \autoref{Main-While-case-1} \textbf{hx.query(\&hx)} é verdadeira.
{
...
\\
\\
O código da interrupção 2 (\autoref{Interrupt_2}) esta abaixo descrita na \autoref{Jump-menu} e serve para mudar de \textit{Menu} e limpar a \acs{eeprom}.
{
